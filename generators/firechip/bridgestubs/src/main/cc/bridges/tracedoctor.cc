      ...
      clock_info(clock_info)
{
  ...
  if (traceEnabled) {
    ...
    // Populate the work queues for each worker
    for (unsigned int i = 0; i < workers.size(); i++) {
      workQueues.push_back(std::queue<referencedBuffer *>());
    }

    for (unsigned int i = 0; i < bufferDepth; i++) {
      std::unique_ptr<referencedBuffer> buffer = std::unique_ptr<referencedBuffer>(new struct referencedBuffer());
    ...
    if (traceThreads > 0) {
      if ((unsigned int) traceThreads >= workers.size()) {
      ...
      auto const &targetWorkFunc = ((unsigned int) traceThreads == workers.size()) ? &tracedoctor_t::balancedWork : &tracedoctor_t::work;
...
void tracedoctor_t::balancedWork(unsigned int const threadId) {
  ...
  while(true) {
    std::unique_lock<locktype_t> queueLock(workQueueLock);
    workQueueCond.wait(queueLock, [this, &myWorkQueue](){return workerExit || !myWorkQueue.empty();});
...
void tracedoctor_t::work(unsigned int const threadId) {
  ...
  while (true) {
    ...
    std::unique_lock<locktype_t> queueLock(workQueueLock);
    workQueueCond.wait(queueLock, [this, &foundJob](){ return workerExit || foundJob || !workQueuesMaybeEmpty; });
